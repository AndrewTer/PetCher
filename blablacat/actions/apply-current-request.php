...
if(isset($_POST["order"]) && isset($_POST["sitter"]) && strlen($_POST["order"])>0 && strlen($_POST["sitter"])>0)
{
        ...
        {
            $del_old_req = "UPDATE request SET deleted = 'yes' WHERE (order_id = $order_id)";
            $add_new_resp = "UPDATE responses SET kind = 'yes' WHERE (order_id = $order_id) AND (sitter_id = $sitter_id)";
        }else
        {
            $del_old_req = "UPDATE request SET deleted = 'yes' WHERE (order_id = $order_id)";
            $add_new_resp = "INSERT INTO responses SET order_id = $order_id, sitter_id = $sitter_id, kind = 'yes'";
        }
